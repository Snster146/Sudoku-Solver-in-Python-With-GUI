...
solveButton=Button(text="Solve",font=("Arial"),command=lambda : find_empty(board))
solveButton.grid(row=14,column=11)

fillButton=Button(text="Fill",font=("Arial"),padx=13,command = lambda : print_unsolved(board))
fillButton.grid(row=15,column=11)
...
resetButton=Button(text="Reset",font=("Arial"),command=reset)
resetButton.grid(row=16,column=11)


def print_unsolved(bo):
    
    for row in range(len(entries)):
        for col in range(len(entries[row])):
            if row < len(bo) and col < len(bo[row]):
                entry_value = entries[row][col].get()
                if entry_value == "":
                    entries[row][col].insert(0,"0")

                # Now you can use entries[row][col] as needed
